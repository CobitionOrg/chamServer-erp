...
export class ExchangeService {
    ...
    async createExchange(createExchangeDto: CreateExchangeDto) {
        const res = await this.prisma.$transaction(async (tx) => {
            const exOrder = await this.exchangeRepository.getExOrder(
                createExchangeDto.id,
                tx,
            );
            ...
            const objOrderBodyType: ExOrderBodyTypeDto =
                exOrder.patient.patientBodyType;
            ...
            //오더 생성
            const newOrder = await this.exchangeRepository.insertOrder(
                tx,
                objOrder,
                createExchangeDto.orderSortNum,
            );
            //오더 아이템 생성
            const newOrderItem = await this.exchangeRepository.insertOrderItems(
                tx,
                objOrderItems,
                newOrder.id,
            );
            ...
            const newOrderBodyType =
                await this.exchangeRepository.insertOrderBodyType(
                    tx,
                    objOrderBodyType,
                    newOrder.id,
                    objOrder.isFirst,
            ...
            if (
                !newOrder.success ||
                !newOrderItem.success ||
                !newOrderBodyType.success
            ) {
                return {
                    success: false,
    ...
    async completeRefund(completeRefundDto: CompleteRefundDto) {
        if (completeRefundDto.orderSortNum !== -4) {
            return {
                ...
                msg: '환불 주문만 완료 처리 가능합니다',
            };
        }
        const res = await this.exchangeRepository.completeRefund(
            completeRefundDto.orderId,
        );
        return res;
    }
}
