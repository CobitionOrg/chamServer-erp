...
export class ErpController {
    ...
    async updateOrder(
        ...
        console.log('??');
        return await this.erpService.updateOrder(
            surveyDto,
            getToken(header),
            id,
        );
    ...
    async callConsulting(
        @Body() callConsultingDto: CallConsultingDto,
        @Headers() header,
    ) {
    ...
    async getCallList(@Headers() header, @Query() getListDto: GetListDto) {
        ...
        const res = await this.erpService.getCallList(
            getToken(header),
            getListDto,
        );
    ...
    async callComplete(
        ...
        const res = await this.erpService.callComplete(
            callConsultingDto,
            getToken(header),
        );
    ...
    async updateOrderByStaff(
        @Body() updateSurveyDto: UpdateSurveyDto,
        @Param('id') id: number,
        @Headers() header,
    ) {
        this.logger.log('직원이 환자 정보 업데이트');
        const res = await this.erpService.updateOrderByStaff(
            updateSurveyDto,
            id,
        );
    ...
    async getCompleeSendOne(
        @Query() sendCompleteDto: SendCompleteDto,
        @Headers() header,
    ) {
    ...
    async updateSendPrice(
        @Body() updateSendPriceDto: UpdateSendPriceDto,
        @Headers() header,
    ) {
        ...
        if (res.success) {
            await this.logService.createLog(
                `${updateSendPriceDto.id}번 발송목록에서 금액만 수정`,
                '발송목록',
                header,
            );
        }
    ...
    @ApiOperation({
        summary: '특정 발송목록을 선택해서 해당 발송목록으로 넘기기',
    })
    @Post('/completeSetSend')
    async completeConsultingSetSend(
        @Body() completeSetSendDto: CompleteSetSendDto,
        @Headers() header,
    ) {
        this.logger.log('특정 발송목록을 선택해서 해당 발송목록으로 넘기기');
        const res =
            await this.erpService.completeConsultingSetSend(completeSetSendDto);
    ...
    @Post('/combine')
    async combineOrder(
        @Body() combineOrderDto: CombineOrderDto,
        @Headers() header,
    ) {
        ...
        if (res.success) {
            await this.logService.createLog(
                `${combineOrderDto.addr}에 ${orderIds}들 합배송`,
                '입금상담목록',
                header,
            );
        }
    ...
    @Post('/send-combine')
    async sendCombine(
        @Body() sendCombineDto: SendCombineDto,
        @Headers() header,
    ) {
        this.logger.log('합배송 처리');
        ...
        if (res.success) {
            await this.logService.createLog(
                `${sendCombineDto.addr}에 ${tempOrderIds}들 합배송`,
                '발송 목록',
                header,
            );
        }
    ...
    async separate(@Body() separateDto: SepareteDto, @Headers() header) {
        ...
        if (res.success) {
            await this.logService.createLog(
                `${separateDto.separate}들에 ${orderIds}를 분리 배송`,
                '입금상담목록',
                header,
            );
        }
    ...
    async addSend(@Body() addSendDto: AddSendDto, @Headers() header) {
        ...
        if (res.success) {
            await this.logService.createLog(
                `${addSendDto.sendListId}를 ${addSendDto.tempOrderId}에 발송일자 변경`,
                '발송목록',
                header,
            );
        }
    ...
    @ApiOperation({
        summary: '발송목록에서 수정하는 데이터 수정 체크 리스트 불러오기',
    })
    ...
    @ApiOperation({
        summary: '체크된 수정 데이터 orderUpdateInfo 테이블에 데이터 넣기',
    })
    @Post('/insertUpdateInfo')
    async insertUpdateInfo(
        @Body() insertUpdateInfoDto: InsertUpdateInfoDto,
        @Headers() header,
    ) {
        this.logger.log(
        ...
        const res =
            await this.sendService.insertUpdateInfo(insertUpdateInfoDto);
        if (res.success) {
            await this.logService.createLog(
                `${insertUpdateInfoDto.tempOrderId}번 데이터 수정`,
                '발송목록',
                header,
            );
        }
    ...
    async checkDiscount(
        ...
        if (res.success) {
            await this.logService.createLog(
                `${checkDiscountDto.orderId}번 지인 확인 할인 여부 체크`,
                '입금상담목록',
                header,
            );
        }
    ...
    async updateNote(@Body() updateNoteDto: UpdateNoteDto, @Headers() header) {
        ...
        if (res.success) {
            await this.logService.createLog(
                `${updateNoteDto.orderId}번 후기 대상 목록에서 비고 수정`,
                '후기대상목록',
                header,
            );
        }
    ...
    async createNewReview(
        @Body() createNewReviewDto: CreateNewReviewDto,
        @Headers() header,
    ) {
    ...
    async updateRouteFlag(
        @Body() routeFlagDto: RouteFlagDto,
        @Headers() header,
    ) {
    ...
    async cancelFriendRecommend(
        @Body() cancelRecommendDto: CancelFriendDto,
        @Headers() header,
    ) {
        this.logger.log('지인 10포 취소 처리');
        const res =
            await this.erpService.cancelFriendRecommend(cancelRecommendDto);
